...
The **ChildWorkflow** recipe code is fully documented in the *AWS Flow
Framework for Ruby Developer Guide*:

-   [Start a Child Workflow Inside a Workflow
    Execution](http://docs.aws.amazon.com/amazonswf/latest/awsrbflowguide/recipes-childworkflow.html)
...
1.  Open a terminal window and change to the `test`{.docutils .literal}
    ...
    ~~~~ {.literal-block}
    cd ~/Downloads/aws-flow-ruby-samples/Recipes/ChildWorkflow/test
    ~~~~
...
3.  Execute the following command on your command-line:
    ...
    ~~~~ {.literal-block}
    rspec order_process_integration_spec.rb
    ~~~~
